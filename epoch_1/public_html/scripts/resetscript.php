...
while ($hofa = mysqli_fetch_array($hofq, MYSQLI_ASSOC)) {
	...
	$offcountq = mysqli_query($db, "SELECT sum(gold) FROM AttackLog{$tbl} WHERE userID=$hofa[ID]") or die("4:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$sumgold = $offcounta[0]; // Amount of gold they've stolen
	$offcountq = mysqli_query($db, "SELECT sum(gold) FROM AttackLog{$tbl} WHERE toUserID=$hofa[ID]") or die("5:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$goldlost = $offcounta[0]; //gold lost
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE toUserID=$hofa[ID] AND type=1") or die("5:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$nuked = $offcounta[0]; //gold
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE attackStrength > defStrength AND userID=$hofa[ID]") or die("6:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$battleswon = $offcounta[0]; //battles won
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE attackStrength < defStrength AND toUserID=$hofa[ID]") or die("7:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$battleswon+= $offcounta[0]; //battles defended
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE attackStrength < defStrength AND userID=$hofa[ID]") or die("8:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$battleslost = $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE attackStrength > defStrength AND toUserID=$hofa[ID]") or die("9:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$battleslost+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT count(*) FROM AttackLog{$tbl} WHERE attackStrength < defStrength AND toUserID=$hofa[ID]") or die("10:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$battlesdef = $offcounta[0];
	$b = ($hofa[sasoldiers] + $hofa[sasoldiers] + $hofa[uu]) * ($conf['gps'][$hofa[race]]);
	$income = $b;
	$thieftscore = 0;
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*50) FROM SpyLog{$tbl} WHERE weapontype2=0 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("20:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*100) FROM SpyLog{$tbl} WHERE weapontype2=1 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("21:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*250) FROM SpyLog{$tbl} WHERE weapontype2=2 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("22:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*500) FROM SpyLog{$tbl} WHERE weapontype2=3 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("23:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*1500) FROM SpyLog{$tbl} WHERE weapontype2=4 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("24:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*3000) FROM SpyLog{$tbl} WHERE weapontype2=5 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("25:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*6000) FROM SpyLog{$tbl} WHERE weapontype2=6 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("26:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*8000) FROM SpyLog{$tbl} WHERE weapontype2=7 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("27:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT (sum(weaponamount)*10000) FROM SpyLog{$tbl} WHERE weapontype2=8 AND isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("28:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftscore+= $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT sum(uu) FROM SpyLog{$tbl} WHERE isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("29:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
	$thieftuu = $offcounta[0];
	$offcountq = mysqli_query($db, "SELECT sum(gold) FROM SpyLog{$tbl} WHERE isSuccess=1 AND type=1 AND userID=$hofa[ID]") or die("30:" . mysqli_error($db));
	$offcounta = mysqli_fetch_array($offcountq);
...
`goldlost`,`battleswon`,`battleslost`,`battlesdefended`, `up`,`income`, `exp`, `numoffiers`,`thieftscore`,`thieftuu`,`thieftgold`,`nuked`) VALUES
 ('" . addslashes($hofa[userName]) . "','$alliance','$hofa[race]','$hofa[bankper]','$hofa[officerup]','$hofa[uu]','$hofa[sasoldiers]',
	...
	$add = mysqli_query($db, $sql) or die("11:" . mysqli_error($db));
}
//clean DB
/* $sql1="UPDATE UserDetails SET fortificationLevel=0,siegeLevel=0,
  ...
  mysqli_query($db, 'TRUNCATE `AtackLog`');
        mysqli_query($db,  ' TRUNCATE `IPs`'); 
        mysqli_query($db,  ' TRUNCATE `Messages`'); 
        mysqli_query($db,  ' TRUNCATE `SpyLog`'); 
        mysqli_query($db,  ' TRUNCATE `Weapon`'); 
        mysqli_query($db,  ' TRUNCATE `click`'); 
        mysqli_query($db,  ' TRUNCATE `iplog`'); 
        mysqli_query($db,  ' TRUNCATE `log`'); 
        mysqli_query($db,  ' TRUNCATE `log_B`'); 
		mysqli_query($db,  ' TRUNCATE `log_C`');
		mysqli_query($db,  ' TRUNCATE `log_D`');
        mysqli_query($db,  ' TRUNCATE `lognew`'); 
        mysqli_query($db,  ' TRUNCATE `outbox`'); 
  
  
 // mysqli_query($db, $sql1) or die("12:".mysqli_error($db));
 mysqli_query($db, "UPDATE Mercenaries SET lastTurnTime='".time()."',attackSpecCount=0,defSpecCount=0");
  
  //Enable
  
 // $configq=mysqli_query($db, "UPDATE `config` SET `value`='false' WHERE `name`='offline'") or die("14:".mysqli_error($db));  */
mysqli_query($db, "update UserDetails set gclick=15,salevel=0,dalevel=0,gold=0,bank=50000,savings=0,attackturns=50,up=0,calevel=0,sflevel=0,sasoldiers=0,samercs=0,dasoldiers=0,damercs=0,uu=200,spies=0,specialforces=0,sa=0,da=0,ca=0,ra=0,hhlevel=0,changenick=0,clicks=0,weapper=0,nukelevel=0,scientists=0,plutonium=0,bunkers=0,nukes=0,nukesteps=0,clickall=0,bankimg=0");
mysqli_query($db, ' TRUNCATE `Messages`');
mysqli_query($db, ' TRUNCATE `SpyLog`');
mysqli_query($db, ' TRUNCATE `Weapon`');
mysqli_query($db, ' TRUNCATE `click`');
...
mysqli_query($db, ' TRUNCATE `outbox`');
mysqli_query($db, ' TRUNCATE `AttackLog`');
mysqli_query($db, 'update alliances set gold=0,exp=0,sa=0,da=0,ca=0,ra=0,up=0, ap=0,usedcash=0,donated=donated+usedcash');
mysqli_query($db, 'update alliances set usedcash=0');
?>
