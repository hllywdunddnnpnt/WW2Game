...
<table width="100%">
	...
	<TR><TD>Forum</TD><td><?php echoURL($alliances[MEMBER]->url, stripslashes($alliances[MEMBER]->url)) ?></td></TR>
...
<?php
$q = mysqli_query($db, "SELECT ID,username,rank FROM UserDetails,Ranks WHERE Ranks.userid=UserDetails.id AND alliance=" . MEMBER . " AND aaccepted=1 AND UserDetails.active=1 ORDER BY rank ASC") or die(mysqli_error($db));
...
<?php
printf("Total: %i Users", mysqli_num_rows($q));
?>
		<Br />
		<hr />
